...
def train_test_Sequence(model, dataset, trainInds, testInds, nEpochs,sampleSeqLength,lr,momentum,samplingEpochs):
	...
	for eph in range(1, nEpochs+1): 
		...
		for i in xrange((nTrainPersons*2)):
			...
			netInputA = Variable(netInputA)
			netInputB = Variable(netInputB)
			...
			target1 = Variable(torch.Tensor([netTarget[0]]))
			target2 = Variable(torch.LongTensor([netTarget[1]]))
			target3 = Variable(torch.LongTensor([netTarget[2]]))
			#loss = nn.HingeEmbeddingLoss(dist, target1) + nn.NLLLoss(logsoftmax1, target2) + nn.NLLLoss(logsoftmax2,target3)
		...
		if eph % 1 == 0:
        			print('Epoch: %d, Batch Error: %.4f, Time=%.4f '%(eph, batchErr.data.item(), toc - tic))
            			batchErr = 0
...
def doDataAug(seq,cropx,cropy,flip):
	seqLen = seq.shape[0]
        ...
        seqDim1 = seq.shape[2]
        seqDim2 = seq.shape[3]
	cropx=int(cropx.item())
	cropy=int(cropy.item())
	flip=flip.item()
